// Extract image[1] (skip screenshot at index 0) from JSONL line 7378
// → convert to 21:8 webp + write to public/media/heros/contact-hero.webp
...
const out = resolve("public/media/heros/contact-hero.webp");
await sharp(buf)
  .resize(W, H, { fit: "cover", position: "centre" })
  .webp({ quality: 82, effort: 5 })
  .toFile(out);
const s = await stat(out);
console.log(`Wrote ${out} (${(s.size/1024).toFixed(1)} KB, ${W}x${H})`);
